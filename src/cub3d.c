...
//         if (*mapX < 0 || *mapX >= game->mapGrid->width || *mapY < 0 || *mapY >= game->mapGrid->height)
//         {
...
#include <string.h> // Ensure ft_strlen is declared
...
t_game *initialize_game(t_vector *map, t_player player, t_assets *assets) {
   t_game *game = malloc(sizeof(t_game));
	...
	game->mlx = NULL;
	game->image = NULL;
	game->mapGrid = map; // Assume map is already allocated
	game->assets = *assets; // Assuming a deep copy is needed
...
//     // Clean up resources after the loop exits
//     cleanup(game);
...
int raycast_engine(t_vector *map, t_player player, t_assets *assets)
{
	t_game *game = initialize_game(map, player, assets);
	if (!game) {
		// Initialization failed, resources are already freed in initialize_game
		return EXIT_FAILURE;
	}
